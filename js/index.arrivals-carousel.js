(function () {
  ...
  function renderProducts(products) {
    ...
    for (const product of products) {
      const productHtml = `
          <article class="arrivals-carousel__item" data-product-id="${product.id}">
            <div class="arrivals-carousel__item-image-wrapper">
              <img
                class="arrivals-carousel__item-image"
                src="${product.image}"
                alt="${product.name}"
              />
            </div>
  ...
  async function loadAndRenderProducts() {
    try {
      ...
      const products = allProducts.filter(p => p.id >= 6 && p.id <= 10);
  ...
  async function initArrivalsCarousel() {
    const arrivalsCarouselSection = document.querySelector(".arrivals-carousel");
    if (!arrivalsCarouselSection) {
      console.warn("Arrivals section not found");
      return;
    }

    const carousel = arrivalsCarouselSection.querySelector(".arrivals-carousel__carousel");
    const track = arrivalsCarouselSection.querySelector(".arrivals-carousel__track");
    let prevBtn = arrivalsCarouselSection.querySelector(".arrivals-carousel__nav--prev");
    let nextBtn = arrivalsCarouselSection.querySelector(".arrivals-carousel__nav--next");

    if (!carousel || !track || !prevBtn || !nextBtn) {
      console.warn("Arrivals carousel elements not found");
      return;
    }
    ...
    const items = track.querySelectorAll(".arrivals-carousel__item");
    if (items.length === 0) {
  ...
  function cloneItems() {
    ...
    originalItems.forEach((item) => {
      ...
      clone.classList.add("arrivals-carousel__item--clone");
  ...
  let allItems = Array.from(track.querySelectorAll(".arrivals-carousel__item"));
  ...
  function updateAllItems() {
    allItems = Array.from(track.querySelectorAll(".arrivals-carousel__item"));
  }
  ...
  if (prevBtn && nextBtn) {
    ...
    console.error("Arrivals carousel buttons not found for event listeners", { prevBtn, nextBtn });
  }
